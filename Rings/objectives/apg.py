...
def APG(models, ob, mcmc_steps, K):
    """
    ...
    for m in range(mcmc_steps):
        ...
        phi_loss_mu, theta_loss_mu, ess_mu, w_mu, mu = Update_mu(f_mu, dec_x, ob, state, angle, mu, K)
        mu = Resample(mu, w_mu, idw_flag=True)
        phi_loss_z, theta_loss_z, ess_z, w_z, state, angle = Update_state_angle(f_state, f_angle, dec_x, ob, state, angle, mu, K)
        ...
        metrics['ess'].append((ess_mu + ess_z).unsqueeze(0) / 2)
    return metrics
