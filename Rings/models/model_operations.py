...
def Init_models(K, D, num_hidden, num_hidden_local, num_nss, RECON_SIGMA, CUDA, device, lr, RESTORE=False, PATH=None):
    ...
    optimizer =  torch.optim.Adam(list(dec_x.parameters())+list(os_mu.parameters())+list(f_state.parameters())+list(f_mu.parameters())+list(f_angle.parameters()),lr=lr, betas=(0.9, 0.99))
