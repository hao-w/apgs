...
class Enc_mu(nn.Module):
    ...
    def forward(self, ob, state, angle, sampled=True, mu_old=None):
        ...
        S, B, N, D = ob.shape
        K = state.shape[-1]
        label = state.argmax(-1).unsqueeze(-1).float()
        ss = self.neural_stats(torch.cat((ob, angle, label), -1))
        gammas = self.gammas(torch.cat((ob, angle, label), -1))
