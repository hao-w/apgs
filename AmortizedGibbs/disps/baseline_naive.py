...
def naive_hmm(Pi, A, T, K):
    Zs = torch.zeros((T, K))
    log_qs = torch.zeros(T)
    for t in range(T):
        if t == 0:
            sample_z = cat(Pi).sample()
            Zs[t] = sample_z
            log_qs[t] = cat(Pi).log_prob(sample_z)
        else:
            labels = Zs[t-1].nonzero().item()
            sample_z = cat(A[labels]).sample()
            Zs[t] = sample_z
...
def naive_sampling(enc, alpha_trans_0, Pi, mu_ks, cov_ks, Z_true, Y, T, D, K, num_particles):
    ...
    for l in range(num_particles):
        ...
        Z, log_qs = naive_hmm(Pi, A_sample, T, K)

        log_p_joints = log_joint(alpha_trans_0, Z, Pi, A_sample, mu_ks, cov_ks, Y, T, D, K).detach()
        log_weights_is[l] = log_p_joints - log_qs - log_q_mlp
        kls[l] = kl_dirichlets(alpha_trans_0, latents_dirs, Z_true, T, K)
