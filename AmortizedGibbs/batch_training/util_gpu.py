...
def log_joints(alpha_trans_0, Z, Pi, As, mu_ks, cov_ks, Ys, T, D, K, batch_size):
    log_probs = torch.zeros(batch_size).float().cuda()
    labels = Z.nonzero().cuda()
    log_probs = log_probs + (MultivariateNormal(mu_ks[labels[:, -1]].view(batch_size, T, D), cov_ks[labels[:, -1]].view(batch_size, T, D, D)).log_prob(Ys)).sum(1).cuda() # likelihood of obs
    log_probs = log_probs + cat(Pi).log_prob(Z[:, 0]).cuda() # z_1 | pi
    inds = labels.view(batch_size, T, 3)[:, :-1, :].contiguous().view(batch_size * (T-1), 3)
    log_probs = log_probs + (cat(As[inds[:, 0], inds[:, -1]].view(batch_size, T-1, K)).log_prob(Z[:, 1:])).sum(1).cuda()
    log_probs = log_probs + (Dirichlet(alpha_trans_0).log_prob(As)).sum(1).cuda() ## prior of A
    return log_probs
