...
def ag_sis_stepwise_adapt(enc, prior_true, As_true, Zs_true, Pi, mu_ks, cov_ks, Ys, T, D, K, num_particles_rws, num_particles_smc, mcmc_steps, batch_size):
    ...
    for m in range(mcmc_steps):
        if m == 0:
            ...
            Zs_prevs = Zs_candidates.transpose(0,1)
            adaptive_reweights = torch.exp(log_uptonow_weights[:, m-1] - logsumexp(log_uptonow_weights[:, m-1], dim=-1).unsqueeze(-1)).detach()
            adaptive_uptonow_weights = (logsumexp(log_uptonow_weights[:, m-1], dim=-1) - torch.log(torch.FloatTensor([num_particles_rws]))).detach()
            for l in range(num_particles_rws):
                ancesters = Categorical(adaptive_reweights).sample()
                Z = Zs_prevs[torch.arange(batch_size), ancesters]
                ## z_pairs (B * T-1)-by-(2K)
                ...
                log_uptonow_weights[:, m, l] = adaptive_uptonow_weights + log_increment_weights[:, m, l]
