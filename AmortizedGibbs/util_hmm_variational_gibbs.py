...
def rws2(enc, alpha_trans_0, Pi, mu_ks, cov_ks, Y, T, D, K, num_particles_rws, num_particles_smc, mcmc_steps):
    ...
    Zs, log_weights, log_normalizers = smc_hmm_v(Pi, As, mu_ks, cov_ks, Y, T, D, K, num_particles_smc, num_particles_rws)
    ...
    for m in range(mcmc_steps):
        ...
        for l in range(num_particles_rws):
            latents_dir, A_sample = enc(Z_ret_pairwise[l], alpha_trans_0.sum().item(), T)
