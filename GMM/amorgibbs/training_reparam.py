...
def test(Eubo, enc_eta, enc_z, Data, K, mcmc_size, sample_size, batch_size, CUDA, device, RESAMPLE):
    NUM_SEQS, N, D = Data.shape
    indices = torch.randperm(NUM_SEQS)
    ...
    if CUDA:
        obs =obs.cuda().to(device)
    symkl_test, eubo_test, elbo_test, _, q_eta, p_eta, q_z, p_z, _, _ = Eubo(enc_eta, enc_z, obs, N, K, D, mcmc_size, sample_size, batch_size, device, RESAMPLE=RESAMPLE)
