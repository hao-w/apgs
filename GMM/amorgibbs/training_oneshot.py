...
def train_cfz(Eubo, oneshot_eta, enc_eta, gibbs_z, optimizer, Data, K, num_epochs, mcmc_size, sample_size, batch_size, PATH, CUDA, device):
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(num_batches):
            ...
            eubos, elbos, esss, q_eta, p_eta, q_z, p_z, q_nu, pr_nu = Eubo(oneshot_eta, enc_eta, gibbs_z, obs, N, K, D, mcmc_size, sample_size, batch_size, device)
...
def train(Eubo, oneshot_eta, enc_eta, enc_z, optimizer, Data, K, num_epochs, mcmc_size, sample_size, batch_size, PATH, CUDA, device):
    EUBOs = []
    ELBOs = []
    ESSs = []
    NUM_SEQS, N, D = Data.shape
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(num_batches):
            ...
            if CUDA:
                obs =obs.cuda().to(device)
            eubos, elbos, esss, q_eta, p_eta, q_z, p_z, q_nu, pr_nu = Eubo(oneshot_eta, enc_eta, enc_z, obs, N, K, D, mcmc_size, sample_size, batch_size, device)
            kl_eta_ex, kl_eta_in, kl_z_ex, kl_z_in = kl_train(q_eta, p_eta, q_z, p_z, q_nu, pr_nu, obs, K)
            ## gradient step
            eubos.sum().backward()
            optimizer.step()
