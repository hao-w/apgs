...
def Init_step_eta_test(models, obs, N, K, D, sample_size, batch_size):
    ...
    q_z, p_z = enc_z.forward(obs, obs_tau, obs_mu, K, sample_size, batch_size)
    ...
    return obs_tau, obs_mu, state, log_weights, q_eta, p_eta, q_z, p_z, log_obs_n.sum(-1).mean()
...
def Incremental_z_test(q_z, p_z, obs, obs_tau, obs_mu, K, D, state_prev):
    ...
    return state, log_w_forward, log_w_backward, log_w_forward.sum(-1).mean()
...
def detailed_balances(log_w_f, log_w_b):
    ...
    loss = (w * log_w_f).sum(0).sum(-1).mean()
