...
def APG(models, ob, mcmc_steps):
    ...
    loss_os, ess_os, w_z, ob_tau, ob_mu, state = Init_eta(os_eta, f_z, ob)
    ...
    for m in range(mcmc_steps):
        if m == 0:
            state = Resample(state, w_z, idw_flag=False) ## resample state
        else:
            state = Resample(state, w_z, idw_flag=True)
        loss_eta, ess_eta, w_eta, ob_tau, ob_mu = Update_eta(f_eta, ob, state, ob_tau, ob_mu)
        ob_tau = Resample(ob_tau, w_eta, idw_flag=True) ## resample precisions
        ob_mu = Resample(ob_mu, w_eta, idw_flag=True) ## resample centers
        loss_z, ess_z, w_z, state = Update_z(f_z, ob, ob_tau, ob_mu, state) ## update cluster assignments
