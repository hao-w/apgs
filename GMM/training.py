...
def train_prior(models, objective, optimizer, data, Model_Params, Train_Params):
    """
    generic training function
    """
    (NUM_EPOCHS, NUM_DATASETS, S, B, CUDA, device, path) = Train_Params
        ...
        for step in range(NUM_BATCHES):
            ...
            for key in metric_step.keys():
                if key in metrics:
                    ...
                    metrics[key] = metric_step[key][-1].item()
            ## compute KL
            kl_step = kl_train_prior(models, obs, reused, EPS)
            for key in kl_step.keys():
                if key in metrics:
                    metrics[key] += kl_step[key]
                else:
...
def kl_train_prior(models, obs, reused, EPS):
