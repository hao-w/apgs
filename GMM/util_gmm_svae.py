...
def logZ(nu, W, beta, D):
    term1 = - (D * (D + 1) / 2) * torch.log(torch.Tensor([2*math.pi])).double()
    term2 = - (nu * D / 2) * torch.log(torch.Tensor([math.pi])).double()
    term3 = - (D / 2) * torch.log(torch.Tensor([beta])).double()
    term4 = - (nu / 2) * torch.log(torch.det(W))
    term5 = (D * (D - 1) / 4) * torch.log(torch.Tensor([math.pi])).double()
    ds = (nu + 1 - (torch.arange(D).double() + 1)) / 2.0
...
def expectedstats(nu_ks, W_ks, m_ks, beta_ks, D, K):
    ...
    niw_stat3 *= -1
    # K
    niw_stat4 = (D / 2) * torch.log(torch.Tensor([2*math.pi])).double() + (1/2) * E_log_W_ks
    niw_stat4 *= -1
...
def kl_niw_2(nu_0, W_0, m_0, beta_0, nu_ks, W_ks, m_ks, beta_ks, D, K):
    ...
    p_niw_eta1, p_niw_eta2, p_niw_eta3, p_niw_eta4 = param_to_natparam(nu_0 * torch.ones(K).double(), W_0.repeat(K, 1, 1), m_0.repeat(K,1), beta_0 * torch.ones(K).double(), D, K)
...
def elbo(log_gammas, alpha_0, nu_0, W_0, m_0, beta_0, N_ks, Y_ks, S_ks, alpha_hat, nu_ks, W_ks, m_ks, beta_ks, Y, N, D, K):
    ...
    kl_phi = kl_niw(nu_0, W_0, m_0, beta_0, nu_ks, W_ks, m_ks, beta_ks, D, K)
    kl_phi2 = kl_niw_2(nu_0, W_0, m_0, beta_0, nu_ks, W_ks, m_ks, beta_ks, D, K)
    print('kl phi1 : %f, kl phi2 : %f' % (kl_phi, kl_phi2))
    ##likelihood term
    log_likelihood = 0.0
    for k in range(K):
