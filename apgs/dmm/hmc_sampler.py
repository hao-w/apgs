...
class HMC():
    ...
    def hmc_sampling(self, enc_local, dec, x, mu, z, beta, trace):
        """
        change of variables beta  = 1 / (1 + exp(-y))
        y = log (beta / (1 - beta))
        """
        for m in range(self.hmc_num_steps):
            mu = self.metrioplis(dec, x, mu=mu.detach(), z=z.detach(), beta=beta.detach())
            q = self.enc_local(x, mu=mu, K=self.K)
            z = q['states'].value
            beta = q['angles'].value
            log_joint = self.log_joint(dec, x, mu=mu, z=z, beta=beta)
    ...
    def log_joint(self, dec, x, mu, z, beta):
        p = dec.forward(x, mu=mu, z=z, beta=beta)
        ll = p['likelihood'].log_prob.sum(-1).sum(-1)
        log_prior_mu = Normal(dec.prior_mu_mu, dec.prior_mu_sigma).log_prob(mu).sum(-1).sum(-1)
        log_prior_z = cat(probs=dec.prior_pi).log_prob(z).sum(-1)
        log_prior_beta = Beta(dec.prior_con1, dec.prior_con0).log_prob(beta).sum(-1).sum(-1)
        return ll + log_prior_mu + log_prior_z + log_prior_beta

    def log_conditional(self, dec, x, mu, z, beta):
        p = dec.forward(x, mu=mu, z=z, beta=beta)
        ll = p['likelihood'].log_prob.sum(-1).sum(-1)
        log_prior_mu = Normal(dec.prior_mu_mu, dec.prior_mu_sigma).log_prob(mu).sum(-1).sum(-1)
        return ll + log_prior_mu

    def metrioplis(self, dec, x, mu, z, beta):
        r_mu = self.init_sample()
        ## compute hamiltonian given original position and momentum
        H_orig = self.hamiltonian(dec, x, mu=mu, z=z, beta=beta, r_mu=r_mu)
        new_mu, new_r_mu = self.leapfrog(dec, x, mu, z, beta, r_mu)
        ## compute hamiltonian given new proposals
        H_new = self.hamiltonian(dec, x, mu=new_mu, z=z, beta=beta, r_mu=new_r_mu)
    ...
    def leapfrog(self, dec, x, mu, z, beta, r_mu):
        for step in range(self.lf_num_steps):
            mu.requires_grad = True
            log_p = self.log_conditional(dec, x, mu, z, beta)
    ...
    def hamiltonian(self, dec, x, mu, z, beta, r_mu):
        """
        compute the Hamiltonian given the position and momntum
        """
        Kp = self.kinetic_energy(r_mu=r_mu)
        Uq = self.log_conditional(dec, x, mu=mu, z=z, beta=beta)
