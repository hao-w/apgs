...
"""
Reparameterized VAE objective in Bouncing MNIST problem
==========
abbreviations:
K -- number of digits
...
def vae_objective(model, AT, frames, mnist_mean, K, loss_required=True, ess_required=True, mode_required=False, density_required=False):
    """
    ...
    z_where, z_what, trace = vae(enc_coor=enc_coor,
                                 dec_coor=dec_coor,
                                 enc_digit=enc_digit,
                                 dec_digit=dec_digit,
                                 AT=AT,
...
#         z_where_t_1 = z_where_t
    ...
    if loss_required:
        elbo = log_w.mean()
        trace['loss'].append((-elbo).unsqueeze(0))
    if ess_required:
        ess = (1. /(w**2).sum(0))
