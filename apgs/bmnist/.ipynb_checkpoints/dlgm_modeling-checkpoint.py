...
def init_model(frame_pixels, digit_pixels, num_hidden_digit, num_hidden_coor, z_where_dim, z_what_dim, CUDA, DEVICE, LOAD_VERSION, LR):
    ...
    if LR is not None:
        optimizer_phi =  torch.optim.Adam(list(enc_coor.parameters())+
                                        # list(dec_coor.parameters())+
                                        list(enc_digit.parameters()),
                                        lr=LR,
                                        betas=(0.9, 0.99))
        optimizer_theta =  torch.optim.Adam(list(dec_digit.parameters()),
                                        lr=LR,
                                        betas=(0.9, 0.99))
        return (enc_coor, dec_coor, enc_digit, dec_digit), optimizer_phi, optimizer_theta
