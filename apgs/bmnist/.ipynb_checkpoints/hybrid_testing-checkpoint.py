...
def test_hybrid_all_v2(model, flags, AT, apg_sweeps, data_paths, mnist_mean_path, T, K, D, z_what_dim, batch_size, sample_size, hmc_num_steps, leapfrog_step_size, leapfrog_num_steps, CUDA, DEVICE):
    metrics = {'apg' : [], 'hmc' : [], "bpg" : []}
    mnist_mean = torch.from_numpy(np.load(mnist_mean_path)).float().unsqueeze(0).repeat(K, 1, 1).unsqueeze(0).unsqueeze(0).repeat(sample_size, batch_size, 1, 1, 1)
    resampler = Resampler(strategy='systematic',
                          sample_size=sample_size,
    ...
    for group, data_path in enumerate(data_paths):
        ...
        for b in range(num_batches):
            ...
            density_dict = hybrid_objective(model=model,
                                            ...
                                            K=K)
            if flags['apg']:
                metrics['apg'].append(density_dict['apg'].mean(-1).mean(-1).cpu().unsqueeze(0))
            if flags['hmc']:
                metrics['hmc'].append(density_dict['hmc'].mean(-1).mean(-1).cpu().unsqueeze(0))
            if flags['bpg']:
                metrics['bpg'].append(density_dict['bpg'].mean(-1).mean(-1).cpu().unsqueeze(0))
            time_end = time.time()
            print('%d / %d in group %d completed (%ds)' % (b+1, num_batches, group, time_end - time_start))
    if flags['apg']:
        np.save('log_joint_apg_%d' % apg_sweeps, torch.cat(metrics['apg'], 0).mean(0).data.numpy())
    if flags['hmc']:
        np.save('log_joint_hmc_%d' % apg_sweeps, torch.cat(metrics['hmc'], 0).mean(0).data.numpy())
    if flags['bpg']:
        np.save('log_joint_bpg_%d' % apg_sweeps, torch.cat(metrics['bpg'], 0).mean(0).data.numpy())
    return metrics