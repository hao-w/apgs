...
class Enc_coor(nn.Module):
    ...
    def forward(self, frames, templates, S, sampled=True, z_where_old=None):
        q = probtorch.Trace()
        B, T, FP, _ = frames.shape
        S, B, DP, _ = templates.shape
        convolved = conv2d(frames.transpose(0, 1), templates.view(S*B, DP, DP).unsqueeze(1), groups=B)
