...
def APG_test(models, frames, tj_b, tj_std, mcmc_steps, mnist_mean, crop):
    ...
    (enc_coor, enc_digit, dec_digit) = models
    E_where, E_what, recon, ess, w_what, z_where, z_what, ll = Init_step(enc_coor, enc_digit, dec_digit, frames, tj_b, tj_std, crop, mnist_mean, training=False)
    ...
    for m in range(mcmc_steps):
        z_what = Resample_what(z_what, w_what)
        E_where, _, ess_where, w_where, z_where, _ = Update_where(enc_coor, dec_digit, frames, tj_b, tj_std, crop, z_what, z_where_old=z_where, training=False)
        z_where = Resample_where(z_where, w_where)
