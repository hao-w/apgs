...
def APG(models, K, D, frames, mcmc_steps, mnist_mean, crop, tj, tj_std):
    """
    ...
    phi_loss, theta_loss, ess, w_what, z_where, z_what = Init_step(tj, tj_std, os_coor, enc_digit, dec_digit, K, D, frames, crop, mnist_mean)
    ...
    for m in range(mcmc_steps):
        ...
        phi_loss_where, theta_loss_where, ess_where, w_where, z_where = Update_where(tj, tj_std, enc_coor, dec_digit, frames, crop, z_what, z_where_old=z_where)
...
def APG_test(models, K, D, frames, mcmc_steps, mnist_mean, crop, tj, tj_std):
    """
    Start with the mnist_mean template,
    and iterate over z_where_t and z_what
    """
    ...
    (os_coor, enc_coor, enc_digit, dec_digit) = models
    E_where, E_what, recon, ess, w_what, z_where, z_what, ll = Init_step(tj, tj_std, os_coor, enc_digit, dec_digit, K, D, frames, crop, mnist_mean, training=False)
    ...
    for m in range(mcmc_steps):
        z_what = Resample_what(z_what, w_what)
        E_where, _, ess_where, w_where, z_where, _ = Update_where(tj, tj_std, enc_coor, dec_digit, frames, crop, z_what, z_where_old=z_where, training=False)
        z_where = Resample_where(z_where, w_where)
