...
class Oneshot_eta(nn.Module):
    def __init__(self, K, D, num_hidden, num_stats, CUDA, device):
        ...
        self.prior_mean_sigma = torch.ones((K, D)) * 3.5
        self.prior_radi_alpha = torch.ones((K, 1)) * 2.0
        self.prior_radi_beta = torch.ones((K, 1)) * 2.0
    ...
    def forward(self, obs, K):
        ...
        radis = Beta(q_radi_alpha, q_radi_beta).sample()
        q.beta(q_radi_alpha,
                q_radi_beta,
                value=radis,
                name='radi')

        p.beta(self.prior_radi_alpha,
