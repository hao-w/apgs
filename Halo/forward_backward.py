...
def Incremental_eta(q_eta, p_eta, obs, state, obs_rad, noise_sigma, K, D, obs_mu_prev):
    ...
    log_q_eta = q_eta['means'].log_prob.sum(-1)
    obs_mu = q_eta['means'].value
    log_obs = True_Log_likelihood(obs, state, obs_mu, obs_rad, noise_sigma, K, D, cluster_flag=True)
    ...
    log_obs_prev = True_Log_likelihood(obs, state, obs_mu_prev, obs_rad, noise_sigma, K, D, cluster_flag=True)
...
def Incremental_z(q_z, p_z, obs, obs_mu, obs_rad, noise_sigma, K, D, state_prev):
    ...
    log_obs_prev = True_Log_likelihood(obs, state_prev, obs_mu, obs_rad, noise_sigma, K, D, cluster_flag=False)
...
def detailed_balances(log_w_f, log_w_b):
    ...
    return symkl_db, eubo_p_qf, gap_gibbs_q, w_sym, w_f
