...
def Init_step_eta(models, obs, noise_sigma, N, K, D, sample_size, batch_size):
    ...
    q_rad, p_rad = enc_rad.forward(obs, obs_mu, noise_sigma, D, sample_size, batch_size)
    log_p_z = q_rad['radius'].log_prob.sum(-1)
    log_q_z = p_rad['radius'].log_prob.sum(-1)
...
def Incremental_z(q_z, p_z, obs, obs_mu, obs_rad, noise_sigma, K, D, state_prev):
    ...
    state = q_z['zs'].value
    log_obs = True_Log_likelihood(obs, state, obs_mu, obs_rad, noise_sigma, K, D, cluster_flag=False, fixed_radius=True)
    ...
    log_obs_prev = True_Log_likelihood(obs, state_prev, obs_mu, obs_rad, noise_sigma, K, D, cluster_flag=False, fixed_radius=True)
...
def detailed_balances(log_w_f, log_w_b, only_forward):
    ...
    if only_forward:
        ...
        gap_gibbs_q = (eubo_p_qf - elbo_p_qf + eubo_p_qb - elbo_p_qb) / 2
