...
def Init_step_z(models, obs, noise_sigma, N, K, D, sample_size, batch_size):
    ...
    log_obs_n = True_Log_likelihood_rad(obs, state, obs_mu, radi, noise_sigma, cluster_flag=True)
...
def Incremental_eta(q_eta, p_eta, obs, state, noise_sigma, obs_mu_prev, radi_prev):
    ...
    ## backward
    log_p_eta_prev = Normal(p_eta['means'].dist.loc, p_eta['means'].dist.scale).log_prob(obs_mu_prev).sum(-1) + Beta(p_eta['radi'].dist.concentration1, p_eta['radi'].dist.concentration0).log_prob(radi_prev).sum(-1)
    log_q_eta_prev = Normal(q_eta['means'].dist.loc, q_eta['means'].dist.scale).log_prob(obs_mu_prev).sum(-1) + Beta(q_eta['radi'].dist.concentration1, q_eta['radi'].dist.concentration0).log_prob(radi_prev).sum(-1)
