...
def train_dec(models, objective_enc, objective_dec, optimizer, optimizer_dec, data, Model_Params, Train_Params):
    """
    generic training function
    """
    (NUM_EPOCHS, NUM_DATASETS, S, B, CUDA, device, path) = Train_Params
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for step in range(NUM_BATCHES):
            ...
            if CUDA:
                obs =obs.cuda().to(device)
            ## update inference networks
            loss, metric_step, reused = objective_enc(models, obs, SubTrain_Params)
            ## gradient step
            loss.backward()
            optimizer.step()
            ## update generative networks
            loss = objective_dec(models, obs, SubTrain_Params)
            ## gradient step
            loss.backward()
            optimizer_dec.step()
            for key in metric_step.keys():
