...
def Eubo_cfz_pr_z(enc_mu, gibbs_z, obs, obs_rad, noise_sigma, N, K, D, mcmc_size, sample_size, batch_size, device, RESAMPLE, DETACH):
    """
    initialize z
...
def Eubo_cfz_pr_eta(enc_mu, gibbs_z, obs, obs_rad, noise_sigma, N, K, D, mcmc_size, sample_size, batch_size, device, RESAMPLE, DETACH):
    """
    initialize eta
...
def Eubo_cfz_os_eta(oneshot_mu, enc_mu, gibbs_z, obs, obs_rad, noise_sigma, N, K, D, mcmc_size, sample_size, batch_size, device, RESAMPLE, DETACH):
    """
    initialize eta
    ...
    elbos[0] = log_weights.sum(-1).mean()
    esss[0] = (1. / (weights**2).sum(0)).mean()
    symkls[0] = (weights * log_weights).sum(0).sum(-1).mean() - log_weights.sum(-1).mean()
