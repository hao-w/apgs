...
def train_rad(Eubo, enc_mu, enc_z, optimizer, Data, K, num_epochs, mcmc_size, sample_size, batch_size, PATH, CUDA, device):
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(num_batches):
            ...
            eubos, elbos, esss = Eubo(enc_mu, enc_z, obs, N, K, D, mcmc_size, sample_size, batch_size, device, noise_sigma=0.05)
            eubos.mean().backward()
            optimizer.step()
            EUBO += eubos[-1].item()
            ELBO += elbos[-1].item()
...
def train(Eubo, enc_mu, enc_z, optimizer, Data, radius, K, num_epochs, mcmc_size, sample_size, batch_size, PATH, CUDA, device):
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(num_batches):
            ...
            if CUDA:
                obs = obs.cuda().to(device)
            eubos, elbos, esss, _, _ = Eubo(enc_mu, enc_z, obs, obs_rad, N, K, D, mcmc_size, sample_size, batch_size, device, noise_sigma=0.05)
            eubos.mean().backward()
            optimizer.step()
            EUBO += eubos[-1].item()
            ELBO += elbos[-1].item()
