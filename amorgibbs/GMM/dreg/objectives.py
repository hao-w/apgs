...
def rws_dreg(enc_init, enc_local, x, N, K, D, SAMPLE_DIM, BATCH_DIM, num_samples, batch_size):
...
def ag_rws(enc_init, enc_global, enc_local, x, N, K, D, SAMPLE_DIM, BATCH_DIM, steps, num_samples, batch_size):
    ...
    eubo = torch.mul(weights, log_incremental_weights).sum(1).sum(0).mean()
    elbo = log_incremental_weights.sum(0).mean(0).mean()
