...
class Enc_mu_rad(nn.Module):
    def __init__(self, K, D, num_hidden, num_stats, CUDA, device):
        ...
        self.mean_mu = nn.Sequential(
            nn.Linear(num_stats+2*D, int(0.5*(num_stats+ 2*D))),
            nn.Tanh(),
            nn.Linear(int(0.5*(num_stats+ 2*D)), D))
        ...
        self.radius_mu = nn.Sequential(
            nn.Linear(num_stats+2, int(0.5*(num_stats+2))),
            nn.Tanh(),
            nn.Linear(int(0.5*(num_stats+2)), 1))
