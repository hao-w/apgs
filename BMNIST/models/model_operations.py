...
def Init_models(K, D, FRAME_PIXELS, DIGIT_PIXELS, HIDDEN_LIST, CUDA, DEVICE, lr, Test=False, RESTORE=False, PATH=None):
    (NUM_HIDDEN_DIGIT, NUM_HIDDEN_COOR, Z_WHAT_DIM) = HIDDEN_LIST
    AT = Affine_Transformer(digit_size=DIGIT_PIXELS, frame_size=FRAME_PIXELS, CUDA=CUDA, DEVICE=DEVICE)
    ...
    dec_coor = Dec_coor(D=D, CUDA=CUDA, DEVICE=DEVICE)
    if CUDA:
        with torch.cuda.device(DEVICE):
            enc_coor.cuda()
            enc_digit.cuda()
            dec_digit.cuda()
    if not Test: ## if initialization is at training time
        if RESTORE:
        ...
        for p in dec_digit.parameters():
            p.requires_grad = False
    return (enc_coor, dec_coor, enc_digit, dec_digit), AT
