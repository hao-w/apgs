...
"""
Sequential Importance Sampling objective in Bouncing MNIST problem
==========
abbreviations:
K -- number of digits
...
def sis_objective(model, AT, apg_sweeps, frames, mnist_mean, K, loss_required=True, ess_required=True, mode_required=False, density_required=False):
    """
    ...
    log_w, z_where, z_what, trace = vae(enc_coor=enc_coor,
                                        dec_coor=dec_coor,
                                        enc_digit=enc_digit,
                                        dec_digit=dec_digit,
                                        AT=AT,
    ...
    for m in range(apg_sweeps):
        log_w, z_where, trace = sis_where(enc_coor=enc_coor,
                                          dec_coor=dec_coor,
                                          dec_digit=dec_digit,
                                          AT=AT,
                                          frames=frames,
                                          ...
                                          density_required=density_required)
        LOG_W = LOG_W + log_w
        log_w, z_what, trace = sis_what(enc_digit=enc_digit,
                                        dec_digit=dec_digit,
                                        AT=AT,
                                        frames=frames,
                                        z_where=z_where,
    ...
    if loss_required:
        assert LOG_W.shape == (S, B), "ERROR! unexpected shape found."
        elbo = LOG_W.mean()
        trace['loss'] = -elbo # (1+apg_sweeps) * 1
    if ess_required:
        w = F.softmax(LOG_W, 0).detach()
        ess = (1. / (w**2).sum(0))
        trace['ess_sis'].append(ess) #   B
...
def vae(enc_coor, dec_coor, enc_digit, dec_digit, AT, frames, digit, trace, loss_required, ess_required, mode_required, density_required):
    T = frames.shape[2]
...
def sis_where(enc_coor, dec_coor, dec_digit, AT, frames, z_what, z_where_old, trace, loss_required, ess_required, mode_required, density_required):
    T = frames.shape[2]
...
#         z_where_old_t_1 = z_where_old_t
        ...
        log_w_sum = log_w_sum + log_w_f - log_w_b  + ll_f.squeeze(-1) - ll_b.squeeze(-1)
    ...
    return log_w_sum, z_where, trace


def sis_what(enc_digit, dec_digit, AT, frames, z_where, z_what_old, trace, loss_required, ess_required, mode_required, density_required):
    ...
    log_w = ll_f.sum(-1) + log_p_f.sum(-1) - log_q_f - (ll_b.sum(-1) + log_p_b.sum(-1) - log_q_b)
