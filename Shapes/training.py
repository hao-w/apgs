...
def train(models, objective, optimizer, data, mcmc_steps, Train_Params, CUDA, device, path):
    ...
    annealed = (torch.arange(mcmc_steps+1) + 1).float() / (mcmc_steps+1)
...
def train_anneal(models, objective, optimizer, data, mcmc_steps, Train_Params, CUDA, device, path):
    """
    training function for datasets with various sizes
    """
    (NUM_EPOCHS, K, D, S, B) = Train_Params
    ...
    NUM_DATASETS = data[0].shape[0]
    NUM_BATCHES = int((NUM_DATASETS / B))
    anneal = (torch.arange(mcmc_steps+1) + 1).float() / (mcmc_steps+1)
    for epoch in range(NUM_EPOCHS):
        Metrics = dict()
        time_start = time.time()
        GROUP_ind = torch.randperm(GROUP_SIZE)
            ...
            for step in range(NUM_BATCHES):
                ...
                if CUDA:
                    with torch.cuda.device(device):
                        ...
                        anneal= anneal.cuda()
                metrics = objective(models, optimizer, ob, mcmc_steps, K)
                phi_loss = (torch.cat(metrics['phi_loss'], 0) * anneal).sum()
                theta_loss = (torch.cat(metrics['theta_loss'], 0) * anneal).sum()
                phi_loss.backward(retain_graph=True)
                theta_loss.backward()
                optimizer.step()
                optimizer.zero_grad()
