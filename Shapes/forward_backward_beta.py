...
def Init_mu(os_mu, f_state, f_angle, dec_x, ob, K, training=True):
    ...
    if training:
        ...
        E_angle = q_angle['angles'].dist.loc.mean(0)[0].cpu().data.numpy()
...
def Update_state_angle(f_state, f_angle, b_state, b_angle, dec_x, ob, state_old, angle_old, mu, K, training=True):
    ...
    q_b_angle, p_b_angle = b_angle(ob, state_old, mu, sampled=False, beta_old=angle_old)
    ...
    if training:
        ...
        E_angle = q_f_angle['angles'].dist.loc.mean(0)[0].cpu().data.numpy()
