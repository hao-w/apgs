...
class HMC():
    ...
    def init_sample(self):
        ...
        return r_mu, r_beta
        """
        return self.gauss_dist.sample((self.S, self.B, self.K, self.D, )).squeeze(-1), self.gauss_dist.sample((self.S, self.B, self.N, ))
    ...
    def hmc_sampling(self, ob, mu, z, beta, trace, hmc_num_steps, leapfrog_step_size, leapfrog_num_steps):
        ...
        for m in range(hmc_num_steps):
            mu, y = self.metrioplis(ob=ob, mu=mu.detach(), y=y.detach(), step_size=leapfrog_step_size, num_steps=leapfrog_num_steps)
            # print(y)
    ...
    def metrioplis(self, ob, mu, y, step_size, num_steps):
        r_mu, r_beta = self.init_sample()
        ## compute hamiltonian given original position and momentum
        H_orig = self.hamiltonian(ob=ob, mu=mu, y=y, r_mu=r_mu, r_beta=r_beta)
        new_mu, new_y, new_r_mu, new_r_beta = self.leapfrog(ob, mu, y, r_mu, r_beta, step_size, num_steps)
        # print(log_tau)
        ## compute hamiltonian given new proposals
        H_new = self.hamiltonian(ob=ob, mu=new_mu, y=new_y, r_mu=new_r_mu, r_beta=new_r_beta)
        ...
        return filtered_mu.detach(), filtered_y.detach()


    def leapfrog(self, ob, mu, y, r_mu, r_beta, step_size, num_steps):
        for step in range(num_steps):
            mu.requires_grad = True
            y.requires_grad = True
            log_p = self.log_marginal(ob, mu, y)
            ...
            mu = mu.detach()
            y = y.detach()
        return mu, y, r_mu, r_beta



    def hamiltonian(self, ob, mu, y, r_mu, r_beta):
        """
        compute the Hamiltonian given the position and momntum

        """
        Kp = self.kinetic_energy(r_mu=r_mu, r_beta=r_beta)
        Uq = self.log_marginal(ob=ob, mu=mu, y=y)
        assert Kp.shape == (self.S, self.B), "ERROR! Kp has unexpected shape."
        assert Uq.shape ==  (self.S, self.B), 'ERROR! Uq has unexpected shape.'
        return Kp + Uq


    def kinetic_energy(self, r_mu, r_beta):
        """
        r_mu : S * B * K * D
        return - 1/2 * ||r_mu+r_beta||^2
        """
        return - ((r_mu ** 2).sum(-1).sum(-1) + (r_beta ** 2).sum(-1).sum(-1)) * 0.5
