...
"""
Sequential Importance Sampling objective in DGMM problem
==========
abbreviations:
...
==========
"""
def sis_objective(model, apg_sweeps, ob, K, loss_required=True, ess_required=True, mode_required=False, density_required=False):
    trace = dict()
    if loss_required:
        trace['loss_phi'] = []
        trace['loss_theta'] = []
    ...
    log_w, w, mu, z, beta, trace = rws(enc_rws_mu=enc_rws_mu,
    ...
    for m in range(apg_sweeps):
        log_w, w, mu, trace = apg_mu(enc_apg_mu=enc_apg_mu,
                                     dec=dec,
                                     log_w=log_w,
                                     ob=ob,
                                     z=z,
        ...
        log_w, w, z, beta, trace = apg_local(enc_apg_local=enc_apg_local,
                                             dec=dec,
                                             log_w=log_w,
                                             ob=ob,
                                             mu=mu,
...
def rws(enc_rws_mu, enc_rws_local, dec, ob, K, trace, loss_required, ess_required, mode_required, density_required):
    ...
    return log_w, w, mu, z, beta, trace

def apg_mu(enc_apg_mu, dec, log_w, ob, z, beta, mu_old, K, trace, loss_required, ess_required, mode_required, density_required):
    """
    ...
    w = F.softmax(log_w, 0).detach()
    if loss_required:
        loss_phi = (w * (- log_q_f).sum(-1)).sum(0).mean()
        loss_theta = (w * (- ll_f_collapsed).sum(-1)).sum(0).mean()
        trace['loss_phi'].append(loss_phi.unsqueeze(0))
        trace['loss_theta'].append(loss_theta.unsqueeze(0))
    if ess_required:
        ess = (1. / (w**2).sum(0))
    ...
    if density_required:
        ...
        return log_w, w, mu, trace


def apg_local(enc_apg_local, dec, log_w, ob, mu, z_old, beta_old, K, trace, loss_required, ess_required, mode_required, density_required):
    """
