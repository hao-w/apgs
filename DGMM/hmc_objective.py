...
class HMC():
    ...
    def y_to_beta(self, y):
        return 1. / (1 + (- y).exp())
    def hmc_sampling(self, ob, mu, z, beta):
        """
        ...
        for m in range(self.hmc_num_steps):
            mu = self.metrioplis(ob=ob, mu=mu.detach(), z=z.detach(), beta=beta.detach())
    ...
    def metrioplis(self, ob, mu, z, beta):
        ...
        new_mu, new_r_mu = self.leapfrog(ob, mu, z, beta, r_mu)
    ...
    def leapfrog(self, ob, mu, z, beta, r_mu):
