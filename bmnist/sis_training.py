...
def train(optimizer, model, AT, apg_sweeps, data_paths, mnist_mean_path, K, num_epochs, sample_size, batch_size, CUDA, DEVICE, MODEL_VERSION):
    """
    ==========
...
#                     converge_file.close()                
                ...
                if loss_required:
                    if 'loss' in metrics:
                        metrics['loss'] += trace['loss'].item()
                    else:
                        metrics['loss'] = trace['loss'].item()

                if ess_required:
                    assert trace['ess_sis'][0].shape == (batch_size, ), 'ERROR! ess_rws has unexpected shape.'
            ...
            log_file = open('../results/sis/log-' + MODEL_VERSION + '.txt', 'a+')
            time_end = time.time()
